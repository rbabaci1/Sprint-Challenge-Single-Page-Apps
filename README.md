...
> Answer: Client side routing allows us to build single-page web applications with a navigation
> without having to refresh the page a make a request to the server as the user navigates
> between pages. With client side routing, we save a lot of computation witch results in a
> better speed and high performance. We also get the chance to reuse component across multiple pages
> or routes without having to request the server each time.
